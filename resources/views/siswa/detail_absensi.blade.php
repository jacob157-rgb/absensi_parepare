            ...
            <div class="p-1.5 min-w-full inline-block align-middle">
                <div class="border-t border-l border-r rounded dark:border-neutral-700">
                    <h2
                        ...
                        <span class="ml-2">DETAIL ABSENSIKU</span>
                    </h2>
            ...
            <table class="whitespace-no-wrap search-table w-full  min-w-full ">
                <thead>
                    <tr
                        ...
                        <th class="px-4 py-3 truncate">Tanggal</th>
                ...
                <tbody class="dark:divide-gray-700 dark:bg-gray-800 divide-y bg-white">
                    @foreach ($dates as $date)
                        @php
                            $checkAbsensi = App\Models\Absensi::getAbsenBySiswa($siswa->id, $date);
                        @endphp
                        <tr class="dark:text-gray-400 text-gray-700">
                            <td class="px-4 py-3 text-sm font-medium capitalize truncate">
                                @if (formatHari($date) == 'Minggu')
                                    <span class="text-red-600">{{ formatTanggalLengkap($date) }}</span>
                                @else
                            ...
                            <td class="px-4 py-3 text-xs font-medium capitalize truncate">
                                @if (isset($existingAbsensi[$date]))
                                    @php
                                        $dayOfWeek = \Carbon\Carbon::parse($date)->translatedFormat('l'); // Example: 'Senin', 'Selasa', etc.
                                        ...
                                        $jamTerlambat = Carbon\Carbon::parse($jam_absen[$hari]->jam_terlambat);
