            ...
            <div class="p-1.5 min-w-full inline-block align-middle">
                <div class="border-t border-l border-r rounded bg-white dark:border-neutral-700">
                    ...
                    <div class="px-4 py-3 mb-8  rounded dark:bg-gray-800">
                        <form action="{{ route('siswa.perizinan.update', $perizinan->id) }}" method="POST"
                                ...
                                <div>
                                    <label class="block text-sm">
                                        ...
                                        <select name="kategori" data-placeholder="kategori" id="kategori" required
                                            ...
                                            <option value="SAKIT" {{ $perizinan->kategori == 'SAKIT' ? 'selected' : '' }}>
                                                SAKIT
                                            </option>
                                            <option value="LAINNYA" {{ $perizinan->kategori != 'SAKIT' ? 'selected' : '' }}>
                                                LAINNYA</option>
                                        </select>
                                        @error('kategori')
                                ...
                                <div>
                                    <label class="block text-sm">
                                        ...
                                        <input type="date" name="tanggal_selesai" id="tanggal_selesai"
                                            value="{{ $perizinan->tanggal_selesai }}"
                                            class="block w-full mt-1 text-sm border-gray-300 rounded dark:text-gray-300 dark:bg-gray-700 focus:shadow-outline-red form-input focus:border-red-400 focus:outline-none"
                                            placeholder="Berikan Alasannya" />
                                ...
                                <div>
                                    <label class="block text-sm">
                                        ...
                                        <input type="file" name="surat_keterangan" id="surat_keterangan"
                                            value="{{ $perizinan->surat_keterangan }}"
                                            class="block w-full mt-1 text-sm border-gray-300 rounded dark:text-gray-300 dark:bg-gray-700 focus:shadow-outline-red form-input focus:border-red-400 focus:outline-none"
                                        ...
                                        @error('surat_keterangan')
                                            <span class="text-xs text-red-600 dark:text-red-400">{{ $message }}</span>
                                        @enderror
                                        <a href="{{ asset('storage/perizinan/' . $perizinan->surat_keterangan) }}"
                                            class="max-w-40 truncate flex whitespace-nowrap mt-3 py-1.5 px-3 rounded-lg text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-800/30 dark:text-blue-500"
                                            target="_blank">
                                            ...
                                            </svg> {{ $perizinan->surat_keterangan }}
                                        </a>
                                    </label>
                                </div>
