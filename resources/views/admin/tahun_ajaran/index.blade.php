...
@section('content')
    <div class="shadow w-full overflow-hidden rounded  bg-white border ">
        ...
        <div class="w-full overflow-x-auto">
            ...
            {{ $tahun_ajaran->links('vendor.pagination.tailwind') }}
        </div>
    </div>
    ...
    <script>
        $(document).on('click', '.tambahBtn', function(e) {
            ...
            let postUrl = '{{ route('tahun_ajaran.store') }}';
            let modalContent = `
                <label class="block text-sm">
                    ...
                    <input
                        class="block w-full rounded mt-1 text-sm {{ $errors->has('tahun') ? 'border-red-600' : 'border-gray-300' }} dark:text-gray-300 dark:bg-gray-700 form-input"
                        placeholder="Masukan Tahun Ajaran" name="tahun" value="{{ old('tahun') }}" required/>
                    @error('tahun')
        ...
        $(document).on('click', '.editBtn', function(e) {
            ...
            let tahun_id = $(this).data('id');
            $.ajax({
                url: `/admin/tahun_ajaran/${tahun_id}`,
                type: "GET",
                success: function(response) {
                    let modalLabel = 'Edit {{ $pages }}';
                    let postUrl = `/admin/tahun_ajaran/${tahun_id}`;
                    let modalContent = `
                        <label class="block text-sm">
